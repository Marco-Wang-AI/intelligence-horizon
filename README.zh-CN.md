# AGI Radar / AGI 小雷达

一个轻松、不吓人的 AGI / ASI 小雷达。

[English README](./README.md)

这个项目想做两件事：

- 记录专家预测和前沿实验室信号：采访、产品发布、模型能力、政策变化、基准变化。
- 收集大家的预测：普通访问者认为 AGI、ASI 什么时候到来，以及自己的行业什么时候会被 AI 大幅自动化。

它不是“AI 末日钟”。我们更想做一个有证据、有分歧、也有一点玩笑感的小项目。

## 当前版本

- 中英双语静态网站。
- 两只雷达：AGI 和 ASI，用小雷达展示最新信号。
- 每只雷达同时显示“专家预测”和“大众感受”。
- 雷达记录：每个事件都有方向、置信度、相对距离和公开来源。
- 科幻时间轴：整理经典作品里的 AI 节点。
- 中文投票使用问卷星，英文投票使用 Google Forms。
- 可选的一句话定义：用户可以写下自己理解的 AGI / ASI，并自主选择是否署名。
- 定义榜单：展示社区里更受欢迎的定义样例。
- 每两周发布一次雷达更新。

## UI 方案预览

`concepts.html` 里放了几个视觉方向：

- 快了雷达。
- 日历融化钟。
- 仪表盘时钟。
- 星图时钟。
- 时间胶片。

## 数据结构

雷达事件放在 `data/events.json`。一条事件大概长这样：

```json
{
  "date": "2026-05-26",
  "title": {
    "en": "DeepMind leadership says society has only a few years to prepare",
    "zh": "DeepMind 负责人称社会只剩几年准备时间"
  },
  "source": {
    "name": "Axios",
    "url": "https://example.com/source"
  },
  "target": "AGI",
  "deltaMeters": -90,
  "confidence": {
    "en": "medium",
    "zh": "中"
  },
  "category": "expert_prediction"
}
```

## 投票问题

当前投票包括：

- 你认为 AGI 什么时候到来？
- 你认为 ASI 什么时候到来？
- 可选：你所在的行业是什么？
- 可选：AI 什么时候能完成你所在领域 90% 的工作？
- 可选：用一句话定义 AGI。
- 可选：用一句话定义 ASI。

首发阶段的数据分别收集在问卷星和 Google Forms，汇总与后端计划写在 `docs/vote-backend.md`。

## 隐私与访问统计

网站使用 Cloudflare Web Analytics 查看聚合访问数据，例如页面浏览量、访问次数、来源网站、国家地区、设备和浏览器。项目不会用统计工具识别单个访问者。

## 如何参与

你可以通过 GitHub issue 或 pull request 参与：

- 提交一个新的雷达事件，并附上公开来源。
- 讨论某个事件应该让雷达靠近还是远离中心。
- 优化中英文文案。
- 改进投票问题。
- 帮忙接入公开投票后端。
- 做每周或每月更新。

## 本地预览

打开 `index.html`，或者在项目目录里启动任意静态服务器。

推广文案草稿在 `docs/launch-copy.md`。

版本更新记录见 [`CHANGELOG.zh-CN.md`](./CHANGELOG.zh-CN.md)。
